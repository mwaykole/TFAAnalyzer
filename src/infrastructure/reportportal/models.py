...
class TestItemWithLogs(BaseModel):
    """Test item with associated log entries."""
    ...
    def get_combined_logs(self) -> str:
        """Get all log messages combined into a single string."""
        log_lines = []
        for log in sorted(self.logs, key=lambda x: x.time or datetime.min):
            timestamp = log.time.isoformat() if log.time else "N/A"
